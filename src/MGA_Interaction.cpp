...
void write_jsmol_sphere_xyz(particle *& particle_list,triplet &L, const int N_rods, string fname, const int attractive_spheres = 15, const int spheres_per_rod = 20, const double length = 5){ // copied From Main program, modified to change particle type to cluster #
  // Outputs the current configuration in a format that can be read by Ovito
	// open the movie file for writing to
  ofstream output_movie;  	// File containing data for VMD movie
	output_movie.open(fname.c_str(),ios::out);
  ...
  //	output_movie.precision(dl::digits10);  // Set coord output to max precision to avoid numerical errors when restarting from old coords
    ...
    if (j < attractive_spheres){
      ...
      s_type = 0;
		...
		output_movie << s_type << " " << (particle_list[i].coords - L/2 + particle_list[i].dir * (-length/2. + sphere_spacing*j)) << "\n";
    }
	}
	// close movie file
	output_movie.close();
